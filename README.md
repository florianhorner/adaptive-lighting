...
> **Versioning:** This fork uses its own version track. **`v1.32.0`** is the first stable on that track. It supersedes `v1.32.0-beta.2` (fork additions plus deliberately ported upstream [#1482](https://github.com/basnijholt/adaptive-lighting/pull/1482) and [#1483](https://github.com/basnijholt/adaptive-lighting/pull/1483)) and includes CI maintenance from [#28](https://github.com/florianhorner/adaptive-lighting-fork/pull/28). The line branched from upstream commit [`6cebe14`](https://github.com/basnijholt/adaptive-lighting/commit/6cebe14a69bf0ea19efeaf3ed5c20e5396087216) (PR [#1426](https://github.com/basnijholt/adaptive-lighting/pull/1426)), not from the final upstream `v1.31.0` tag. Fork and upstream versions with the same number are not the same code.
...
| `expand_light_groups`          | Expand light groups to their individual member entities (`true`, default). Set to `false` to send adaptation commands to the group entity directly instead of its members.                                                                                                                                                                                                                    | `True`         | `bool`                                  |
...
| `manual_control`         | Pause adaptation for both axes, only brightness, only color, or resume adaptation entirely. Accepts boolean for legacy compatibility (true=pause both, false=resume). | ❌         | bool or one of `['brightness', 'color']` |
...
- **Per-light diagnostic status sensors** — see when AL is active, blocked, or overridden on each light (opt-in)

### Bug fixes (contributed back upstream)
- **Entity ID duplication on HA 2026.4+** — fix by [@stephanGarland](https://github.com/stephanGarland) ([upstream #1459](https://github.com/basnijholt/adaptive-lighting/issues/1459))
- **Transient Zigbee off-state cancelling adaptation** — prevents brief off-states from resetting manual control
- **Log verbosity for self-triggered off-to-on warning** — reduces noisy debug output
- **Allow `sleep_brightness=0`** — by [@jaynis](https://github.com/jaynis) ([upstream #1455](https://github.com/basnijholt/adaptive-lighting/pull/1455))
