...
async def test_get_astral_observer_compatibility(hass):
    """Use the native or compatibility observer with identical coordinates."""
    observer = get_astral_observer(hass)
    assert observer.latitude == hass.config.latitude
    assert observer.longitude == hass.config.longitude
    assert observer.elevation == hass.config.elevation
...
@pytest.mark.parametrize(
    "expand_light_groups",
    [True, False],
    ids=["expanded-group", "preserved-group"],
)
async def test_just_turned_off_group_context_reuse_end_to_end(
    hass,
    cleanup,
    expand_light_groups,
):
    ...
    events on the bus. It covers both the legacy expanded-group path and the
    fork's Lightener/direct-group mode, where only the group is managed and a
    member turn-on must still be retained as explanatory evidence.
    """
    from homeassistant.components.adaptive_lighting.const import (
    ...
    if expand_light_groups:
        ...
        assert member not in manager.lights
    ...
    if expand_light_groups:
        # The fork's full-depth expansion re-runs during reset and removes an
        # available group. Re-add it to model the issue's nested/unavailable
        # setup, where the group entity itself remains tracked after the event.
        manager.lights.add(group)
    ...
    if not expand_light_groups:
        assert member not in manager.lights
        assert member not in manager.manual_control
...
async def test_unsupported_skip_warns_once_per_light(hass, caplog):
    """Empty-data skip warns once per light and re-arms after switch restart.
    ...
    with patch(
        ...
        with caplog.at_level(logging.WARNING):
            third = await switch.prepare_adaptation_data(
                ENTITY_LIGHT_1,
                transition=0,
                adapt_brightness=True,
                adapt_color=True,
        ...
        assert len(rearmed_warning_records) == 1
